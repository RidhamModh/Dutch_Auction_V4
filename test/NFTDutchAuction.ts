...
describe("Auction", function () {
  ...
  async function deployDutchAuctionTestFixture() 
  {
    ...
    const auctionFactory = await ethers.getContractFactory("NFTDutchAuction_ERC20Bids");
  ...
  {
      it("Check if the NFT Ownership is successfully transferred in the auction contract", async function () 
      {
      ...
      it("Check if the NFT is transferred in the auction smart contract", async function () 
      {
        ...
        await expect(auctionContract.connect(owner).cancelAuction()).revertedWith("Auction NFT is not transferred.");
      });
      ...
      it('should revert if cancelAuction is called before NFT is transferred', async () => {
        const { auctionContract} = await loadFixture(deployDutchAuctionTestFixture);
        await expect(auctionContract.cancelAuction()).to.be.revertedWith('Auction NFT is not transferred.');
      });
      ...
      it("should not allow to end auction if it is already completed", async function () {
        ...
        await auctionContract.setAuctionHasEnded(true);
        
        // Try to end the auction and expect it to fail
        await expect(auctionContract.connect(firstAcc).cancelAuction()).to.be.revertedWith("Cannot halt the auction as it is successfully completed.");
    });
  ...
  describe("Upgradable", () => 
  {
    ...
    {
      const { auctionContract } = await loadFixture(deployDutchAuctionTestFixture);
      const auctionContractUpgrade = await ethers.getContractFactory("NFTDutchAuction_Upgraded");
      const auctionContractUpgradeDeploy = await upgrades.upgradeProxy(auctionContract.address, auctionContractUpgrade);

      expect(await auctionContractUpgradeDeploy.currentVersion()).to.equal(ethers.BigNumber.from("5"))
    });
